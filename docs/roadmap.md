...
**最後更新：2026-09-04**　　580+ 個測試通過（含真實 broker 端到端）。
...
**~~把 `/live` 與 `/workout` 的 CSS 與字型改為本機提供~~** — 已完成（2026-09-01）。全站靜態資源（Tailwind `app.css`、Google Fonts）已改由本機伺服，無網路環境亦能正常離線渲染，經 `tests/offline.rs` 驗證。
